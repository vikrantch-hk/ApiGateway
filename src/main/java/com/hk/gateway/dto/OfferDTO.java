...
import com.hk.gateway.response.es.ESOfferResponse;
...
public class OfferDTO extends JSONObject {
	 ...
	 * 
	 * this.id = offer.getId(); this.storeId = offer.getStoreId(); this.name =
	 * offer.getName(); this.offerTypeId = offer.getOfferTypeId(); this.startDt =
	 * offer.getStartDt(); this.endDt = offer.getEndDt(); this.offerPaymentModeId =
	 * offer.getOfferPaymentModeId(); this.terms = offer.getTerms();
	 * 
	 ...
	 * this.timesAllowedPerUserId = offer.getAllowedTimesPerUserId(); if
	 * (offer.getRoles() != null) { for (Role role : offer.getRoles()) { String
	 * roleName = role.getName(); offerRoles.add(roleName); } } }
	 */
	...
	public boolean equals(Object o) {
		...
		return new EqualsBuilder().append(id, offerDTO.id).append(name, offerDTO.name)
				.append(offerTypeId, offerDTO.offerTypeId).append(couponCode, offerDTO.couponCode).isEquals();
	}

	@Override
	public int hashCode() {
		return new HashCodeBuilder(17, 37).append(id).append(name).append(offerTypeId).append(couponCode).toHashCode();
	}
}
