...
import com.hk.gateway.constant.EnumImageFormat;
import com.hk.gateway.constant.ImageAlignmentEnum;
import com.hk.gateway.constant.ImageSizeEnum;
import com.hk.gateway.constant.ImageTypeConstants;
import com.hk.gateway.constant.Keys;
import com.hk.gateway.constant.StoreConfigKeys;
import com.hk.gateway.dto.ImageDTO;
import com.hk.gateway.exception.InvalidParameterException;
import com.hk.gateway.service.StoreService;

/**
 * Created by IntelliJ IDEA. User: Vaibhav Date: Apr 10, 2013 Time: 1:56:50 AM
 */
@Component
public class HKImageUtil {
	...
	public String getS3ImageUrl(Long storeId, Long imageFormat, ImageSizeEnum imageSize,
			ImageAlignmentEnum imageAlignmentEnum, String amazonImageId) {
		StringBuilder imageUrl = new StringBuilder();
		String prefix = "http://";

		String imageDistributionUrl = getImageDistributionUrl(storeId, amazonImageId);
		imageUrl.append(prefix).append(imageDistributionUrl)
				.append(getS3ImageKey(imageSize, imageFormat, imageAlignmentEnum, amazonImageId));

		return imageUrl.toString();
	}

	private static String getS3ImageKey(ImageSizeEnum imageSize, Long imageFormat,
			ImageAlignmentEnum imageAlignmentEnum, String amazonImageId) {
		...
		EnumImageFormat enumImageFormat = EnumImageFormat.getImageFormatEnumById(imageFormat) == null
				? EnumImageFormat.JPG
				: EnumImageFormat.getImageFormatEnumById(imageFormat);
		imageKey.append(amazonImageId).append("_");
		if (imageAlignmentEnum != null) {
			imageKey.append(imageAlignmentEnum.getSuffix()).append("_");
		}
		imageKey.append(imageSize.getSuffix()).append("." + enumImageFormat.getExtension());
	...
	private String getImageDistributionUrl(Long storeId, String amazonImageId) {
		...
		// if image distribution prefix and suffix are not blank, then serve images from
		// cdn, else direct from download bucket
		if (StringUtils.isNotBlank(imageDistPrefix) && StringUtils.isNotBlank(imageDistSuffix)) {
			imageDistributionUrl.append(imageDistPrefix).append(getCdnNumber(amazonImageId)).append(".")
					.append(imageDistSuffix).append("/");
		} else {
			...
			imageDistributionUrl.append(downloadBucket).append(".s3.amazonaws.com/");
		}
	...
	public static void syncImageInfo(ImageDTO imageDTO) throws IOException {
		...
		try {
			...
			logger.error("Error while converting image to buffered image: " + imageDTO.getImageFile().getName() + ": "
					+ iioe);
		}
	...
	public static String getAmazonImageId(int imageType, Long imageId) {
		...
		imageKey.append(imageId / noOfImagesInRepo + 1).append("/").append(getImagePrefix(imageType)).append(imageId);
	 ...
	 * { if (tempImage == null) { throw new
	 * InvalidParameterException("FILE_BEAN_CANNOT_BE_NULL"); }
	 * 
	 * File imageFile = new File(TEMP_IMAGE_DIRECTORY + System.currentTimeMillis() +
	 * "_" + BaseUtils.getRandomString(4) + ".jpg");
	 * imageFile.getParentFile().mkdirs(); tempImage.save(imageFile);
	 * 
	 * return imageFile; }
	 */
	...
	public File getImage(String amazonImageId, Long storeId, Long imageFormat) {
		EnumImageFormat enumImageFormat = EnumImageFormat.getImageFormatEnumById(imageFormat) == null
				? EnumImageFormat.JPG
				: EnumImageFormat.getImageFormatEnumById(imageFormat);
		File imageFile = new File(TEMP_IMAGE_DIRECTORY + System.currentTimeMillis() + "_" + BaseUtils.getRandomString(4)
				+ "." + enumImageFormat.getExtension());
	...
	public static boolean isJpegImage(String imagePath) {
		...
		return StringUtils.isNotBlank(imageFileType)
				&& (imageFileType.equalsIgnoreCase("jpg") || imageFileType.equalsIgnoreCase("jpeg"));
	}
